...
- Финальный выбор для MVP: `Pack B` (`theme-b`) — зафиксирован как базовая тема.
...
## Решение по MVP (зафиксировано)
1. `Theme Pack`: `B` (`theme-b`) — основной.
2. Плотность интерфейса: `normal`.
3. Стиль кнопок: `строгие`.
4. Таблицы по умолчанию: `zebra`.

`theme-a` и `theme-c` оставлены как альтернативные профили для A/B-тестов и будущей кастомизации.
